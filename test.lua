...
--  Visual:  Orion-style UICorner + UIStroke  |  Linoria two-column layout
--  Theme:   Yuri dark (maintarget.lua colours) as default
--  Addons:  SaveManager inlined at bottom
-- =====================================================================
...
-- Flat global stores (SaveManager reads these directly)
local Toggles = {}
local Options = {}

-- MARK: ─── Library singleton ──────────────────────────────────────────
local Library = {
    -- Registry
    Registry      = {};
    RegistryMap   = {};
    ...
    -- ── Yuri dark theme (exact maintarget.lua palette) ────────────────
    BackgroundColor      = Color3.fromRGB(10,  10,  10);   -- root bg
    MainColor            = Color3.fromRGB(18,  18,  18);   -- panel / groupbox bg
    SecondaryColor       = Color3.fromRGB(28,  28,  28);   -- button / input bg
    SecondaryHoverColor  = Color3.fromRGB(42,  42,  42);   -- button hover
    OutlineColor         = Color3.fromRGB(45,  45,  45);   -- borders
    FontColor            = Color3.fromRGB(200, 200, 200);  -- body text
    AccentFontColor      = Color3.fromRGB(220, 220, 220);  -- title / accent text
    DimColor             = Color3.fromRGB(80,  80,  80);   -- placeholder / dim
    AccentColor          = Color3.fromRGB(220, 220, 220);  -- accent highlight
    AccentDarkColor      = Color3.fromRGB(160, 160, 160);  -- accent dimmed
    ...
    -- Global widget stores (exposed for SaveManager)
    Toggles  = Toggles;
    Options  = Options;
...
-- Box: Frame + UICorner + UIStroke (Orion-style)
...
function Library:Unload()
    ...
    getgenv_().YuriLib = nil
end

-- MARK: ─── DependencyBoxes ────────────────────────────────────────────
...
-- =====================================================================
-- MARK: ─── BaseFuncs mixin ────────────────────────────────────────────
--  Applied (via setmetatable) to every container: Groupbox, Section,
--  DependencyBox, Tabbox sub-tab.
--  Each container must expose: .Container (Frame + UIListLayout)
--                              :Resize()
-- =====================================================================
local BaseFuncs  = {}
...
function BaseFuncs:AddLabel(idx, text, wrap)
    -- Overload: (idx, text, wrap) | (text, wrap) | (text)
    ...
    -- Right-aligned addon slot (KeyPicker / ColorPicker swatches)
...
function BaseFuncs:AddButton(config, fn2)
    ...
    -- Sub-button: chops the row 50/50, returns a second button API
...
function BaseFuncs:AddToggle(idx, info)
    ...
    -- Invisible hit region covers full row
    ...
    hit.InputBegan:Connect(function(input)
        ...
        and input.UserInputType ~= Enum.UserInputType.Touch then return end
        if Library:MouseOverOpenedFrame() then return end
        -- Don't fire when clicking an addon swatch / keybox
...
function BaseFuncs:AddDropdown(idx, info)
    ...
    -- Floating list (parented to ScreenGui so it draws above everything)
...
-- ── AddSection (inline titled sub-group, no outer border) ─────────────
function BaseFuncs:AddSection(config)
    ...
    -- Returns a proxy that shares the parent container
...
-- ── AddTable (column-header data grid) ────────────────────────────────
...
-- ── AddConsole (append-only scrolling log) ────────────────────────────
...
-- ── AddDependencyBox (show/hide container based on Toggle state) ───────
...
-- =====================================================================
-- MARK: ─── BaseAddons (ColorPicker, KeyPicker) ────────────────────────
--  Applied to Toggle and Label via setmetatable(obj, BaseAddons).
--  BaseFuncs forwards AddColorPicker / AddKeyPicker so containers can
--  also call them for standalone widgets.
-- =====================================================================
BaseAddons          = {}
...
function BaseAddons:AddColorPicker(idx, info)
    ...
    -- Small colour swatch button in label's right-aligned slot
    ...
    local open = false
    local pf   = nil        -- picker frame (lazy build)
    ...
    local function BuildPicker()
        ...
        -- Reposition if swatch moves (window drag)
        ...
        -- Transparency bar (optional — only if config supplied it)
        local aBar, aCursor
...
function BaseAddons:AddKeyPicker(idx, info)
    ...
    -- Small keybind box in label's right-aligned slot
...
-- Mirror onto BaseFuncs so groupboxes can call them standalone
...
function Library:CreateWindow(config)
    config = config or {}
    local title     = config.Title or "YuriLib"
    ...
    local HDR  = Library.IsMobile and 54 or 60    -- header height
    local PAD  = 12
    ...
    if keySys.Enabled then
        ...
        gGui.Name = "YuriLib_Gate"; gGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        ...
        Corner(6, gHdr)
        Library:Create("Frame", {   -- fills bottom rounded gap
            BackgroundColor3=Library.MainColor; BorderSizePixel=0;
        ...
        -- Intro tween (Back ease, matches maintarget.lua exactly)
    ...
    -- Wrapper drives position / drag; inner `outer` tweens size
    ...
    local outer = Library:Create("Frame", {
        ...
        BorderSizePixel  = 0;
        Size             = UDim2.fromOffset(0, 0);   -- tweens in below
        ClipsDescendants = true;
        ZIndex           = 1;
        Parent           = wrapper;
    })
    ...
    -- Horizontal tab strip (Linoria-style)
    ...
    -- Main content area (below tab strip)
    ...
    -- Minimize-to-bar (Orion-style, Ketamine net-new idea)
    ...
    function Window:AddTab(name, _tabConfig)
        ...
        local function AddGroupbox(gname, side)
            ...
            -- 2px accent stripe (Linoria visual signature)
            local stripe = Library:Create("Frame", {
                BackgroundColor3=Library.AccentColor; BorderSizePixel=0;
                Size=UDim2.new(1,0,0,2); ZIndex=5; Parent=gOuter
            })
            Corner(5, stripe)
            Library:Create("Frame", {  -- fill bottom gap of stripe's rounded corners
                BackgroundColor3=Library.AccentColor; BorderSizePixel=0;
        ...
        -- Tabbox: mini sub-tabs inside a column (same widget vocab via BaseFuncs)
